...
<body>
    ...
    $affected_aeronef = $aeronefRepository->getAeronef($order->aeronef_id)
    ?>
    <div class="col-sm-9 col-xs-12 content pt-3 pl-0">
        <h5 class="mb-3"><strong>Update an order</strong></h5>
        <div class="row">
            <div class="col-sm-12">
                <div class="mt-4 mb-3 p-3  button-container bg-white border shadow-sm">
                    <h6 class="mb-4">ORDER</h6>
                    <form action="index.php?action=updateOrder&order_id=<?= $order->order_id ?>" method="post">
                        <div class="form-group row">
                            <label class="control-label col-sm-3" for="PN">PART NUMBER</label>
                            <div class="col-sm-5">
                                <input type="text" value='<?= $order->part_number ?>' class="form-control" id="PN" name="PN" required />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-sm-3" for="NAME">NAME</label>
                            <div class="col-sm-5">
                                <input type="text" value='<?= $order->name ?>' class="form-control" id="name" name="name" required />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-sm-3" for="supplier">SUPPLIER</label>
                            <div class="col-sm-5">
                                <input type="text" value='<?= $order->supplier ?>' class="form-control" id="supplier" name="supplier" required />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-sm-3" for="quantity">QUANTITY</label>
                            <div class="col-sm-5">
                                <input type="number" min="0" class="form-control" value='<?= $order->quantity ?>' id="quantity" name="quantity" required />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-sm-3" for="price">PRICE</label>
                            <div class="col-sm-5">
                                <input type="number" value='<?= $order->price ?>' min="0" class="form-control" id="price" name="price" required />
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-5">
                                <input type="date" value='<?= $order->order_date ?>' class="form-control" id="ORDER_DATE" name="ORDER_DATE" required />
                            </div>
                        </div>
                        <div class="form-group row">
